...
struct impf_LinearConstraint constraints[] = {
	{ "", constraint_1_coef,  3., impf_CONS_T_EQ },
	{ "", constraint_2_coef,  2., impf_CONS_T_EQ },
	{ "", constraint_3_coef, -5., impf_CONS_T_EQ },
	{ "", constraint_4_coef,  1., impf_CONS_T_EQ }
};
...
int main(void)
{
	...
	double x[n], value;
	int code;
	int state = impf_lp_simplex(objective, constraints, NULL, m, n, "bland", 1000, x, &value, &code);
	...
	assert(__impf_ABS__(x[2]) < 1e-8);
	assert(__impf_ABS__(x[3] - 1.) < 1e-8);
	return 0;
}
